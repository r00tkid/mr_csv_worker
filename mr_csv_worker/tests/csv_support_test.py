...
class CsvWorkerTest(unittest.TestCase):
    worker = CsvWorker()
    test_file_4_read = os.path.join(os.path.dirname(__file__),
                                                            'resources/test_bonus_source_file.csv')
    test_file_4_write = os.path.join(os.path.dirname(__file__), 'results/test.csv')

    def test_get_rows_list_from_csv(self):
        rows = self.worker.get_rows_list_from_csv(csv_file_path=self.test_file_4_read,
                                                  encoding='cp1251', start_row=1, end_row=2)
    ...
    def test_write_rows_from_dict(self):
        ...
        self.worker.write_rows_from_list_of_dict(csv_file_path=self.test_file_4_write,
                                                 fields_names_list=field_names,
